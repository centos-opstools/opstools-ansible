...
- `firewall_data` (default: `null`)

    A lists of hashes containing data for configuration firewall rules
    to be created on each host groups

- `rabbit_hosts` (default: `[{"protocol": "tcp", "port": "{{ rabbitmq_port }}"}, {"protocol": "tcp", "port": "{{ rabbitmq_ssl_port }}"}]`)

    RabbitMQ for sensu

- `redis_hosts` (default: `[{"protocol": "tcp", "port": "{{ redis_listen_port }}"}]`)

    Redis for Sensu

- `sensu_hosts` (default: `[{"protocol": "tcp", "port": "{{ sensu_api_port }}"}]`)

    Sensu API for public access

- `uchiwa_hosts` (default: `[{"source": "{{ uchiwa_bind }}", "protocol": "tcp", "port": "{{ uchiwa_port }}"}]`)

    Uchiwa web UI for public access

- `elastic_hosts` (default: `[{"protocol": "tcp", "port": "{{ elasticsearch_port }}"}]`)

    Fluentd aggregator for public access

- `kibana_hosts` (default: `[{"source": "{{ kibana_server_bind }}", "protocol": "tcp", "port": "{{ kibana_server_port }}"}, {"protocol": "tcp", "port": "{{ opstools_apache_http_port }}"}, {"protocol": "tcp", "port": "{{ opstools_apache_https_port }}"}]`)

    Kibana server

- `grafana_hosts` (default: `[{"protocol": "tcp", "port": "{{ opstools_apache_http_port }}"}, {"protocol": "tcp", "port": "{{ opstools_apache_https_port }}"}]`)

    Grafana

- `graphite_hosts` (default: `[{"protocol": "tcp", "port": "{{ graphite_port }}"}]`)

    Graphite

- `collectd_hosts` (default: `[{"protocol": "tcp", "port": "{{ collectd_listen_port }}"}]`)
